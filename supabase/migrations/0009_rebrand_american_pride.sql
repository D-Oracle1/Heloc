-- Rebrand seeded deposit data from US Bank to American Pride Bank (demo).
update public.claims set source = 'American Pride Bank' where source = 'US Bank';
update public.claims set destination = 'American Pride Bank' where destination = 'US Bank';
update public.claims set note = 'Opening HELOC credit (demo)' where note = 'Refund for Gerrald Butler''s HELOC';

-- Update signup seeding to the new brand + neutral demo note.
...
begin
  ...
  values
    (new.id,
     ...
     'Opening HELOC credit (demo)', 'American Pride Bank', 'in', 'American Pride Bank')
